...
async def get_sources_for_user(
	...
	if user is None:
		raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
	return await source_service.get_sources_for_user(session, user_id, limit=pagination.limit, offset=pagination.offset)
...
async def get_all_sources(
	...
	return await source_service.get_all_sources(session, limit=pagination.limit, offset=pagination.offset)
